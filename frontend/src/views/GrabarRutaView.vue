  ...
  export default {
    ...
    methods: {
          ...
          if (statusInicio === 'OK' && resultsInicio.length) {
            ...
            geocoder.geocode({ location: puntoFinal }, (resultsFin, statusFin) => {
              if (statusFin === 'OK' && resultsFin.length) {
                ...
                this.ruta.ubicacion =
                  puebloInicio === puebloFin ? puebloInicio : `${puebloInicio} – ${puebloFin}`
      ...
      cancelar() {
        ...
        if (
          confirm(
            '¿Estás seguro de que quieres cancelar y recargar la página? Se perderán los datos no guardados.',
          )
        ) {
          location.reload()
        }
